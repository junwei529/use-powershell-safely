...
EXPECTED_SOURCE_MAPPING_SHA256 = 'b7018a4088c294ccd3830ee22ab5460f0acabf65f1dc4498eb9bd032931bba60'
...
def run_adversarial_matrix() -> int:
    ...
    def change_current_tree(repo):
        path = repo / "release" / "v0.3.4-candidate.json"
    ...
    execute_case("missing-current-candidate",
                 lambda repo: (repo / "release" / "v0.3.4-candidate.json").unlink(),
    ...
    execute_case("missing-current-map",
                 lambda repo: (repo / "provenance" / "source-map-v0.3.4.json").unlink(),
    ...
    def mutate_manifest(repo, change):
        path = repo / "provenance" / "source-map-v0.3.4.json"
...
def main() -> int:
    ...
    manifest_path = ROOT / "provenance" / "source-map-v0.3.4.json"
    if has_link_like_component(manifest_path):
        result = {
            "product": PRODUCT,
            "source_commit": BASELINE,
            "mapped_files": 0,
            "failures": ["provenance/source-map-v0.3.4.json: symlink or junction path is not allowed"],
            "result": "FAIL",
        }
    ...
    try:
        ...
        result = {
            ...
            "source_commit": BASELINE,
            "mapped_files": 0,
            "failures": [f"provenance/source-map-v0.3.4.json: unreadable or invalid JSON: {error}"],
            "result": "FAIL",
        }
        print(json.dumps(result, ensure_ascii=False, sort_keys=True) if args.json else f"{PRODUCT}: FAIL")
        return 1
    ...
    if contains_unpaired_unicode_surrogate(manifest):
        result = {
            ...
            "failures": ["provenance/source-map-v0.3.4.json: unpaired Unicode surrogate is not allowed"],
            "result": "FAIL",
        }
        print(json.dumps(result, ensure_ascii=False, sort_keys=True) if args.json else f"{PRODUCT}: FAIL")
        return 1
    ...
    expected_files = {value for value in valid_destinations if is_safe_repo_path(value)} | {"provenance/source-map-v0.3.4.json"}
    ...
    if manifest_raw.startswith(b"\xef\xbb\xbf"):
        failures.append("provenance/source-map-v0.3.4.json: UTF-8 BOM is not allowed")
    for number, line in enumerate(manifest_text.splitlines(), start=1):
        if line.endswith((" ", "\t")):
            failures.append(f"provenance/source-map-v0.3.4.json:{number}: trailing whitespace")
    ...
    for label in sorted(manifest_private_labels):
        failures.append(f"provenance/source-map-v0.3.4.json: {label}")
